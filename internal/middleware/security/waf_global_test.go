...
func TestCreateGlobalWAF_LoadsCRSWithDefaultFlags(t *testing.T) {
	...
	mw, err := NewGlobalWAF(Deps{GlobalStore: store})
	if err != nil {
		t.Fatalf("NewGlobalWAF: %v", err)
	}
	if mw == nil {
...
func TestCreateGlobalWAF_AllowsGRPC(t *testing.T) {
	...
	mw, err := NewGlobalWAF(Deps{GlobalStore: store, RouteType: "grpc"})
	if err != nil {
		t.Fatalf("NewGlobalWAF: %v", err)
	}
...
func TestCreateGlobalWAF_GRPCRelaxationNotBypassableByHeader(t *testing.T) {
	...
	mw, err := NewGlobalWAF(Deps{GlobalStore: store, RouteType: "http"})
	if err != nil {
		t.Fatalf("NewGlobalWAF: %v", err)
	}
	handler := mw(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
...
func TestCreateGlobalWAF_DisabledReturnsNil(t *testing.T) {
	...
	for name, cfg := range cases {
		t.Run(name, func(t *testing.T) {
			mw, err := NewGlobalWAF(Deps{GlobalStore: &mockGlobalConfigStore{config: cfg}})
			if err != nil {
				t.Fatalf("NewGlobalWAF: %v", err)
			}
			if mw != nil {

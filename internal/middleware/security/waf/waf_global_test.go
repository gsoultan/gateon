...
func TestCreateGlobalWAF_LoadsCRSWithDefaultFlags(t *testing.T) {
	...
	mw, err := NewGlobalWAF(security.Deps{GlobalStore: store})
	if err != nil {
...
func TestCreateGlobalWAF_GRPCRelaxationNotBypassableByHeader(t *testing.T) {
	...
	mw, err := NewGlobalWAF(security.Deps{GlobalStore: store, RouteType: "http"})
	if err != nil {
		t.Fatalf("NewGlobalWAF: %v", err)
	}
	handler := mw(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
...
func TestCreateGlobalWAF_DisabledReturnsNil(t *testing.T) {
	...
	for name, cfg := range cases {
		t.Run(name, func(t *testing.T) {
			mw, err := NewGlobalWAF(security.Deps{GlobalStore: &mockGlobalConfigStore{config: cfg}})
			if err != nil {
				t.Fatalf("NewGlobalWAF: %v", err)
			}
			if mw != nil {

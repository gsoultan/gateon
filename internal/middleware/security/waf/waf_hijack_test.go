...
func TestResponseWriterWrappersPreserveHijacker(t *testing.T) {
	wrappers := map[string]http.ResponseWriter{
		// This table has now been split twice by ADR-0002, and the halves are
		// named so neither can be quietly dropped. transformResponseWriter
		// went to internal/middleware/transform with BodyTransform, guarded
		// there by TestTransformResponseWriterPreservesHijacker.
		// deceptionResponseWriter and breadcrumbWriter stayed in
		// internal/middleware/security with deception.go and honeypot.go,
		// guarded there by TestSecurityResponseWriterWrappersPreserveHijacker.

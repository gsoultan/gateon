...
func TestWAFThreatSeverityUsesTheDashboardVocabulary(t *testing.T) {
	cases := []struct {
		...
		{types.SeverityCritical, severityCritical},
		{types.SeverityError, severityHigh},
		{types.SeverityWarning, severityMedium},
		{types.SeverityNotice, severityLow},
	}
...
func TestWAFSeverityIsRankedByTheCorrelationEngine(t *testing.T) {
	...
	if inc.Severity != severityHigh {
		t.Fatalf("incident severity %q from three %q WAF blocks, want %q", inc.Severity, severity, severityHigh)
	}
}

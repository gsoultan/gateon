...
func (s *TLSOptionServiceImpl) ListPaginated(page, pageSize int32, search string) ([]*gateonv1.TLSOption, int32) {
	return s.store.ListPaginated(page, pageSize, search)
}

// SaveTLSOption validates, assigns ID if needed, and persists.
func (s *TLSOptionServiceImpl) SaveTLSOption(opt *gateonv1.TLSOption) error {
	if opt.Id == "" {
		opt.Id = uuid.NewString()
	}
	return s.store.Update(opt)
}

// DeleteTLSOption removes the TLS option.
func (s *TLSOptionServiceImpl) DeleteTLSOption(id string) error {
	if id == "" {

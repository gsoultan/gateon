...
func (s *Store) Seed(ctx context.Context) error {
	initialRules := []Rule{
		...
		},
		{
			...
			Directive:     `SecRule ARGS|REQUEST_HEADERS|REQUEST_FILENAME|XML:/* "@pm betting gambling casino slot machine poker sportsbook jackpot lottery bookmaker odds payout wagering baccarat blackjack roulette" "id:210001,phase:2,block,t:none,t:lowercase,msg:'Online Gambling Site Detected',logdata:'Matched Data: %{MATCHED_VAR}',severity:'CRITICAL',tag:'gambling'"`,
			Enabled:       true,
			ParanoiaLevel: 1,
		...
		},
		{
			...
			Directive:     `SecRule ARGS|REQUEST_HEADERS|XML:/* "@pm <?php file_get_contents( include( require( eval( exec( system( passthru( shell_exec( base64_decode( $_GET $_POST $_REQUEST $_SERVER $_COOKIE $_FILES" "id:210003,phase:2,block,t:none,t:lowercase,msg:'PHP Vulnerability Exploit Attempt',logdata:'Matched Data: %{MATCHED_VAR}',severity:'CRITICAL',tag:'php',tag:'injection'"`,
			Enabled:       true,
			ParanoiaLevel: 1,
			Category:      "Injection",
		},
		{
			...
			Directive:     `SecRule REQUEST_FILENAME "@rx \.(php[345]?|phtml|asp[x]?|jsp[x]?|sh|py|pl|exe|cgi|htaccess)$" "id:210004,phase:2,block,t:none,t:lowercase,msg:'Arbitrary File Upload Execution Attempt',logdata:'Matched Data: %{MATCHED_VAR}',severity:'CRITICAL',tag:'fileupload',tag:'rce'"`,
			Enabled:       true,
			ParanoiaLevel: 1,
			Category:      "Injection",
		},
  ...
  SecRule REQUEST_URI "!@rx ^(/api/|/v[0-9]/)"`,
		...
		},
		{
			...
			Directive:     `SecRule ARGS|REQUEST_BODY "@rx (__proto__|constructor\[prototype\]|\.prototype\.)" "id:170001,phase:2,deny,status:403,msg:'Advanced Prototype Pollution attempt',tag:'attack-generic',severity:CRITICAL"`,
			Enabled:       true,
			ParanoiaLevel: 1,
			Category:      "Exploit",
		},
		{
			...
			Directive:     `SecRule FILES_NAMES|ARGS_NAMES "@rx (?i)\.\./" "id:151003,phase:2,deny,status:403,msg:'Potential Apache Struts Path Traversal (CVE-2023-50164)',tag:'attack-rce',tag:'cve-2023-50164',severity:CRITICAL"`,
			Enabled:       true,
			ParanoiaLevel: 1,
			Category:      "RCE",
		},
		{
			...
			Directive:     `SecRule ARGS|REQUEST_BODY "@rx (?:#|\\$)\\{.*\\}" "id:151004,phase:2,deny,status:403,msg:'Potential Spring Cloud Gateway SpEL Injection (CVE-2022-22947)',tag:'attack-rce',tag:'cve-2022-22947',severity:CRITICAL"`,
			Enabled:       true,
			ParanoiaLevel: 1,
			Category:      "RCE",
		},
		{
			...
			Directive:     `SecRule REQUEST_URI "@rx /goanywhere/licensing/accept" "id:151005,phase:1,deny,status:403,msg:'Potential Fortra GoAnywhere MFT RCE attempt (CVE-2023-0669)',tag:'attack-rce',tag:'cve-2023-0669',severity:CRITICAL"`,
			Enabled:       true,
			ParanoiaLevel: 1,
			Category:      "RCE",
		},
		{
			...
			Directive:     `SecRule REQUEST_HEADERS:X-siis-session-id ".*" "id:140003,phase:1,deny,status:403,msg:'Potential MOVEit Transfer SQLi attempt (CVE-2023-34362)',tag:'attack-sqli',tag:'cve-2023-34362',severity:CRITICAL"`,
			Enabled:       true,
			ParanoiaLevel: 1,
			Category:      "SQLi",
		},
		{
			...
			Directive:     `SecRule &REQUEST_HEADERS:Content-Length "@gt 1" "id:120012,phase:1,deny,status:400,msg:'HTTP Request Smuggling: Multiple Content-Length headers',tag:'ddos',tag:'protocol',severity:CRITICAL"`,
			Enabled:       true,
			ParanoiaLevel: 1,
			Category:      "DoS",
		},
		{
			...
			Directive:     `SecRule ARGS "@rx (?i)(?:127\.0\.0\.1|localhost|169\.254\.169\.254|metadata\.google\.internal|kubernetes\.default\.svc)" "id:150003,phase:2,deny,status:403,msg:'Potential SSRF attempt (Internal target)',tag:'attack-ssrf',severity:CRITICAL"`,
			Enabled:       true,
			ParanoiaLevel: 1,
			Category:      "Protocol",
		},
		{
			...
			Directive:     `SecRule REQUEST_URI "@rx /(?:\.env|\.git|\.vscode|\.idea|docker-compose\.yml|config\.php\.bak)" "id:110004,phase:1,deny,status:403,msg:'Sensitive file discovery attempt',tag:'scanner',tag:'recon',severity:CRITICAL"`,
			Enabled:       true,
			ParanoiaLevel: 1,
			Category:      "Scanner",
		},
		{
			...
			Directive:     `SecRule ARGS|REQUEST_BODY "@rx \$\{(?:script|dns|url):" "id:151006,phase:2,deny,status:403,msg:'Potential Text4Shell RCE attempt (CVE-2022-42889)',tag:'attack-rce',tag:'cve-2022-42889',severity:CRITICAL"`,
			Enabled:       true,
			ParanoiaLevel: 1,
			Category:      "RCE",
		},
		{
			...
			Directive:     `SecRule ARGS|REQUEST_BODY "@rx ms-msdt:/id\s+IT_BrowseForFile" "id:151007,phase:2,deny,status:403,msg:'Potential Follina RCE attempt (CVE-2022-30190)',tag:'attack-rce',tag:'cve-2022-30190',severity:CRITICAL"`,
			Enabled:       true,
			ParanoiaLevel: 1,
			Category:      "RCE",
		},
		{
			...
			Directive:     `SecRule ARGS|REQUEST_BODY "@rx (?:;|\||&|\$|\n|\r|` + "`" + `)\s*(?:env|set|export|eval|exec|system|passthru|shell_exec|base64|python|perl|ruby|php|gcc|make|apt-get|yum|dnf)\b" "id:151008,phase:2,deny,status:403,msg:'Advanced Shell Injection attempt',tag:'attack-rce',severity:CRITICAL"`,
			Enabled:       true,
			ParanoiaLevel: 1,
			Category:      "RCE",
		},
		{
			...
			Directive:     `SecRule ARGS:__amp_source_origin "!@rx ^https?://%{REQUEST_HEADERS:Host}" "id:180000,phase:2,deny,status:403,msg:'AMP Source Origin spoofing attempt',tag:'attack-amp',severity:CRITICAL"`,
			Enabled:       true,
			ParanoiaLevel: 1,
			Category:      "Exploit",
		},
		{
			...
			Directive:     `SecRule ARGS|REQUEST_BODY "@rx (?:coinhive\.min\.js|authedmine\.min\.js|cryptonight\.wasm|monerominer|miner\.start)" "id:190000,phase:2,deny,status:403,msg:'Malicious script/miner detected',tag:'attack-malware',severity:CRITICAL"`,
			Enabled:       true,
			ParanoiaLevel: 1,
			Category:      "Exploit",
		},

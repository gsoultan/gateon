...
func init() {
	...
	Register(31, "add_disabled_and_2fa_pending_to_users", func(db *sql.DB, dialect Dialect) error {
		var queries []string
		switch dialect.Driver {
		case DriverPostgres:
			queries = []string{
		...
		default: // sqlite
			queries = []string{
				...
				`ALTER TABLE users ADD COLUMN two_factor_pending BOOLEAN DEFAULT FALSE;`,
			}
		}
	...
	Register(36, "add_triggered_rules_to_security_threats", func(db *sql.DB, dialect Dialect) error {
		var query string
		switch dialect.Driver {
		case DriverPostgres:
